...
class UsersController extends Controller
{
    ...
    public function show()
    {
        ...
        $users = $this->userService->todos($empresa_id_filter);
...
public function update($id, Request $request)
{
    try {
        ...
        if (Auth::user()->cargo == 'master') {
            $cargo = $request->cargo;
            $empresa_id = $request->empresa;
            $this->userService->editar($id, $name, $cargo, $empresa_id, $tipo);
        } else {
            // Se não for master, o cargo não muda. Usamos o que já existe no banco.
            $cargo = $userToUpdate->cargo;
            $this->userService->editar($id, $name, $cargo, null, $tipo);
        }
    ...
    public function destroy(Request $request)
    {
        try {
            ...
            $userToDelete = $this->userService->buscaId($userId);

            // LÓGICA DE SEGURANÇA: Impede admin de excluir usuário de outra empresa
            if (Auth::user()->cargo != 'master' && $userToDelete->empresa_id != Auth::user()->empresa_id) {
                abort(403, 'Acesso não autorizado');
            }
