...
class ImportProducts extends Component
{
    ...
    protected function createProductItem($item)
    {
        $this->itemProd[] = ['cProd' => (string) $item->prod->cProd[0], 'xProd' => (string) $item->prod->xProd[0],
            ...
            'uCom' => (string) $item->prod->uCom[0], 'qCom' => (string) $item->prod->qCom[0], 'vTotalProd' => (string) $item->prod->vProd[0],
            'vProd' => (string) $item->prod->vUnCom[0], 'ICMS' => (string) $item->imposto->ICMS->ICMS10->pICMS,
